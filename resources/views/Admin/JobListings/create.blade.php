...
@section('content')
    ...
    <div class="max-w-7xl mx-auto bg-white dark:bg-gray-800 p-6 md:p-8 rounded-lg shadow">
        ...
        <form action="{{ route('admin.jobs.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <!-- Status -->
            <div class="mb-4">
                <label for="title" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Status
                    (e.g., Kontrak, Karyawan Tetap)</label>
                    </label>
            ...
            <!-- External LInk -->
            <div class="mb-4">
                <label for="type" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">External Links
                    (e.g., Google forms, bit.ly)</label>
                <input type="text" name="external_link" id="external_link" value="{{ old('external_link') }}"
                    class="mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-gray-900 dark:text-gray-100">
                @error('external_link')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                <label for="postedDate" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Posted
                    Date</label>
                <input type="date" name="postedDate" id="postedDate" value="{{ old('postedDate', date('Y-m-d')) }}" required
                    class="mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-gray-900 dark:text-gray-100">
                @error('postedDate')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" rows="5" required
                    class="summernote mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-gray-900 dark:text-gray-100">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
